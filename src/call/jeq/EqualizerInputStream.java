...
/**
 * The EqualizerInputStream class Author: Dmitry Vaguine Date: 02.05.2004 Time:
 * 12:00:29
 */
public class EqualizerInputStream extends InputStream {
	 ...
	 * 
	 * @param stream
	 *        is an input stream for pcm data
	 * @param samplerate
	 *        is a sample rate of input data
	 * @param channels
	 *        is the number of channels
	 * @param signed
	 *        indicates that the data is signed
	 * @param samplesize
	 *        is the sample bit size of data
	 * @param bigendian
	 *        indicates that the dat is in "big endian" encoding
	 * @param bands
	 *        is the number of bands
	 */
	public EqualizerInputStream(InputStream stream, float samplerate, int channels, boolean signed,
			int samplesize, boolean bigendian, int bands) {
		this.stream = stream;
		...
		case 24: {
			...
			if (l > 0) {
				if (bigendian)
					...
					for (int i = 0; i < l; i++) {
						temp = (((a1 = inbuf)[inpos++] & 0xff) << 16) | ((a1[inpos++] & 0xff) << 8)
								| (a1[inpos++] & 0xff);
						workbuf[i] = signed && temp > 8388607 ? temp - 16777216 : temp;
					}
				inlen -= inpos;
