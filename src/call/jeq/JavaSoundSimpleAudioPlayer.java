...
public class JavaSoundSimpleAudioPlayer {
	...
	public static void playAudioStream(AudioInputStream audioInputStream) {
		...
		try {
			...
			EqualizerAudioInputStream eq = new EqualizerAudioInputStream(audioInputStream, 31);
			for (int i = 1; i < 31; ++i)eq.getControls().setBandValue(i, 0, eq.getControls().getMinimumBandValue());
